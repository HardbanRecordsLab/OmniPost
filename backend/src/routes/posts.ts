    ...
    async (request, reply) => {
      try {
        ...
        if (isNaN(parsed.getTime())) {
          reply.statusCode = 400;
          return { error: 'scheduledAt must be a valid ISO8601 date' };
        }

        const result = await pool.query(
          'UPDATE posts SET scheduled_at = $1, updated_at = NOW() WHERE id = $2 AND user_id = $3 RETURNING *',
          [parsed.toISOString(), id, userId]
        );
        ...
        return { message: 'Post rescheduled successfully', post: result.rows[0] };
      } catch (error) {
        fastify.log.error(error);
        reply.statusCode = 500;
        return { error: 'Internal server error' };
