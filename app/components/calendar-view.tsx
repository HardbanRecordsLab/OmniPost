...
function DroppableCell({
  ...
  return (
    <div
      ref={setNodeRef}
      className={cn(
        "min-h-[140px] rounded-xl border p-3 bg-card hover:bg-muted/40 transition-colors relative",
        isOver && isOutsideWindow && "border-yellow-400",
        isOver && !isOutsideWindow && "border-primary",
...
function DroppableTimeSlot({
  ...
  const { setNodeRef, isOver } = useDroppable({
    id,
    data: { date, time: label },
  })
  return (
    <div
      ref={setNodeRef}
      className={cn(
        "min-h-10 p-1 rounded-lg transition-colors",
        isOver && isOutsideWindow && "border border-yellow-400 bg-yellow-400/10",
        isOver && !isOutsideWindow && "border border-primary/50 bg-primary/5",
        !isOver && "bg-muted/20"
      )}
    >
      {children}
...
export function CalendarView({ onCreatePost }: CalendarViewProps) {
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    const lbl = time || post.time || "09:00"
    const allowed = isSlotAllowed(lbl, post)

    if (!allowed) {
      const confirmed = window.confirm(
        `This time slot (${lbl}) is outside the publishing window for one or more platforms. Schedule anyway?`
      )
      if (!confirmed) return
    }
    ...
    // Optimistic update
    const prevState = { ...postsByDate }
    setPostsByDate(prev => {
      const next: PostsByDate = {}
      for (const [key, posts] of Object.entries(prev)) {
        next[key] = posts.filter(p => p.id !== post.id)
      }
      const newKey = format(date, "yyyy-MM-dd")
      const updatedPost = { ...post, time: lbl }
      next[newKey] = [...(next[newKey] || []), updatedPost]
      return next
    })

    try {
      await api.reschedulePost(post.id, iso)
      window.dispatchEvent(new Event("posts:refresh"))
    } catch {
      // Rollback on error
      setPostsByDate(prevState)
    }
  }
      ...
      <div className="p-6 space-y-6">
        ...
        <div className={cn("grid gap-3", viewMode === "day" ? "grid-cols-1" : "grid-cols-7")}>
          {calendarDays.map((date, index) => {
            ...
            const cellOutside = !isSlotAllowed(defaultTime, activeDragPost)
            ...
            return (
              ...
              >
                {viewMode === "day" ? (
                  // Day view: individual droppable time slots
                  <div className="rounded-xl border border-border p-3 bg-card space-y-1">
                    ...
                    {Array.from({ length: 96 }, (_, i) => i).map((i) => {
                      ...
                      const slotOutside = !isSlotAllowed(label, activeDragPost)
                      return (
                        <DroppableTimeSlot
                          key={i}
                          id={`${dateKey}-${label}`}
                                ...
                                {postsAtSlot.map((post) => (
                                  <DraggablePost key={post.id} post={post}>
                                    <motion.div
                                      ...
                                      animate={{ opacity: 1, y: 0 }}
                                      exit={{ opacity: 0, y: -6 }}
                                      className="p-2 rounded-lg bg-muted/40 hover:bg-muted flex items-center gap-2"
                                      onClick={() => {
                                    ...
                                    <button
                                      ...
                                      onClick={async () => {
                                        try {
                                          ...
                                          await api.reschedulePost(selectedEdit.post.id, iso)
                  ...
                  // Month/week view: one droppable cell per day
                  <DroppableCell
                    id={dateKey}
                    date={date}
                    time={activeDragPost?.time || "09:00"}
                    isOutsideWindow={cellOutside}
                  >
                    <div className="flex items-center justify-between mb-2">
                      <span className={cn("text-sm font-medium", isCurrentDay && "text-primary")}>
                        {format(date, "d")}
