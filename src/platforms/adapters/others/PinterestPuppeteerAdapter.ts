...
export class PinterestPuppeteerAdapter extends PuppeteerAdapter {
  constructor() {
  ...
  protected async detectLoginChallenge(page: Page): Promise<boolean> {
    const el = await page.$('input[id="email"]');
    return el !== null;
  }

  protected async fillPostContent(page: Page, post: Post): Promise<void> {
    await this.antiBan.humanType(page, 'textarea[placeholder]', post.content);
  }
