...
export class TelegramPuppeteerAdapter extends PuppeteerAdapter {
  constructor() {
  ...
  protected async detectLoginChallenge(page: Page): Promise<boolean> {
    const el = await page.$('input.input-field-input');
    return el !== null;
  }

  protected async fillPostContent(page: Page, post: Post): Promise<void> {
    await this.antiBan.humanType(page, 'div.input-message-input', post.content);
  }
